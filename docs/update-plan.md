...
当前版本已实现手动检查、双源结果展示、下载进度、取消、SHA-256 校验、单文件替换
和失败回滚。自动定时检查暂未启用。当前软件版本以项目清单文件为准，不在本文重复
维护，以免说明与程序版本脱节。
...
- 历史记录：Windows 已完成 `0.1.4` 到 `0.1.5` 的真实跨版本下载、替换和重启；
  当前发布版本仍需按下列场景回归，Linux 仍需完成真实升级验收。
